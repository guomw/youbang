...
namespace msite.OAuth2
{
    ...
    /// </summary>
    public partial class WeixinCallback : basicPage
    {
        string _redirectUrl = "";
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            _redirectUrl = UserLogic.Instance.GetOAuthUrl(this.NonceStr);
            ...
            try
            {
                ...
                //获取openid                
                atResult = OAuthApi.GetAccessToken(this.AppId(this.CustomerId), this.AppSecret(this.CustomerId), this.Code);
                ...
                try
                {                    
                    OAuthUserInfo result = OAuthApi.GetUserInfo(atResult.access_token, atResult.openid);
                    LogHelper.Log(Newtonsoft.Json.JsonConvert.SerializeObject(result));
                    if (result != null)
                    {
                ...
                {
                    LogHelper.Log("OAuth.GetAccessToken:" + ex.Message, LogHelperTag.ERROR);
                ...
                try
                {
                    ...
                    {
                        string encodedUserInfo = HttpContext.Current.Server.UrlEncode(JsonHelper.JsonSerializer(oauthUserInfo));
                    ...
                    if (TryReg)
                    {
                        ...
                        int retuid = UserLogic.Instance.GetTopOAuthedUserId(this.CustomerId, atResult.openid);
                        if (retuid == 0)//注册
                        {
                            /**
                             * 执行用户注册操作
                             */
                            retuid = UserLogic.Instance.RegisterUser(wouInfo);
                        }
        ...
        {
            string content = this.LoadErrorNote("404", str);
